...
export default function WidgetSelector({ initialWidgets, initialTheme }: Props) {
  // selected as ordered array. empty array = default (show assigned items)
  ...
  useEffect(() => {
    if (!initialWidgets || initialWidgets.length === 0) {
      ...
      setSelected(Array.from(initialWidgets));
    }
  }, [initialWidgets]);
  ...
  return (
    <div className="p-3 border rounded mb-4 bg-white/5">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="text-sm text-muted">Selecciona y ordena los widgets. Vacío = por defecto (ver asignado).</div>
      </div>
      ...
      <div className="mb-3">
        ...
        {selected.length === 0 && <div className="text-sm text-muted">(Vacío — se mostrará el comportamiento por defecto)</div>}
        <ul className="space-y-1 mt-2">
          {selected.map((k, idx) => {
            const label = AVAILABLE_WIDGETS.find(w => w.key === k)?.label || k;
            return (
              <li key={k} className="flex items-center justify-between gap-2 p-2 border rounded bg-white/3">
