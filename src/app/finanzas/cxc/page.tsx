...
export default function CxCPage() {
  ...
  const { data: resp } = useSWR(`/api/finanzas/cxc?desde=${yearStart}&hasta=${yearEnd}&page=${page}&pageSize=${pageSize}`, fetcher);
  ...
  const byProject = Array.isArray(resp?.byProject) ? resp.byProject : [];
  const bySub = Array.isArray(resp?.bySub) ? resp.bySub : [];
  ...
  const subcontractorMap = useMemo(() => {
    const m = new Map<string,string>();
    if (!Array.isArray(voluntarios)) return m;
    voluntarios.forEach((s: any) => {
      const id = s._id || s.id;
      if (!id) return;
  ...
  const clientOptions = useMemo(() => {
    ...
    // prefer clients from clientes API, else derive from existing items
    if (Array.isArray(clientesList) && clientesList.length) {
      clientesList.forEach((c:any) => {
        const name = c.nombre || c.name || c.display || c.label;
        if (name) set.add(String(name));
      });
    }
    (items || []).forEach((it:any) => { if (it.cliente) set.add(String(it.cliente)); });
  ...
  const clearFilters = () => { setClientFilter(''); setProjectFilter(''); setEstadoFilter(''); };

  // keep localInvoices in sync when initial items load
  React.useEffect(() => { setLocalInvoices(items || []); }, [items]);
  // when page or pageSize changes, ensure we reset editing state
  ...
  // prepare a display list with proyectoName, apply search filter and sorting
  const displayInvoices = useMemo(() => {
    const list = (localInvoices || []).map((r:any) => ({ ...r, proyectoName: proyectoMap.get(String(r.proyectoId)) || '' }));

    const termClient = (clientFilter || '').trim().toLowerCase();
    ...
    const filtered = list.filter((inv:any) => {
      if (termClient && !String(inv.cliente || '').toLowerCase().includes(termClient)) return false;
      if (termProject) {
        const proj = String(inv.proyectoName || inv.proyectoId || '').toLowerCase();
        if (!proj.includes(termProject)) return false; // contains
      }
    ...
    return sorted;
  }, [localInvoices, proyectoMap, sortKey, sortDir, clientFilter, projectFilter, estadoFilter]);

  // formatted display list: use localInvoices (merged) - ensure numbers
  const finalDisplayInvoices = displayInvoices.map((r:any) => ({ ...r }));

  // helper: compute total and balance (used by modal)
  const computeTotal = () => {
  ...
  const computeBalance = (total:number) => {
    // For now assume no payments, balance = total. Extend to fetch payments per invoice if available.
    return total;
  };
  ...
  async function submitInvoice(e?: React.FormEvent) {
    ...
    if (!editingId) {
      ...
      try {
        ...
        console.error('persist cxC invoice error', err);
      }
    } else {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">Cuenta por Cobrar (CxC) - {new Date().getFullYear()}</h1>
        <div className="flex items-center gap-2">
          <button className="btn" onClick={() => setModalOpen(true)}>Nuevo CxC</button>
      ...
      <div className="mb-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          >
            ...
            {clientOptions.map((c:any)=> <option key={c} value={c}>{c}</option>)}
          </select>
      ...
      {/* Invoices table created/entered via modal */}
      <div className="mb-4">
          ...
          <table className="w-full text-sm">
            <thead className="bg-white text-left">
              <tr>
                ...
                <th className="p-2"><button className="w-full text-left" onClick={()=>handleSort('cliente')} aria-label="Ordenar por Cliente">Cliente {sortKey === 'cliente' ? (sortDir === 'asc' ? '▲' : '▼') : ''}</button></th>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <form onSubmit={submitInvoice} className="relative z-50 bg-white p-4 rounded shadow w-[95vw] max-w-2xl">
            <h3 className="text-lg font-semibold mb-2">Nuevo registro CxC</h3>
